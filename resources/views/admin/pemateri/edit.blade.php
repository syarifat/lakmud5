    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-8">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-8">
                ...
                <form action="{{ route('admin.pemateri.update', $pemateri->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="nama" name="nama" type="text" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->nama" required />
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="tempat_lahir" name="tempat_lahir" type="text" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->tempat_lahir" required />
                            </div>
                            
                            <div>
                                <x-input-label for="tanggal_lahir" value="Tanggal Lahir" />
                                <x-text-input id="tanggal_lahir" name="tanggal_lahir" type="date" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->tanggal_lahir" required />
                            </div>

                            <div>
                                <x-input-label for="no_telp" value="Nomor HP / WhatsApp" />
                                <x-text-input id="no_telp" name="no_telp" type="text" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->no_telp" required />
                            </div>

                            <div>
                                <x-input-label for="instagram" value="Akun Instagram (Opsional)" />
                                <x-text-input id="instagram" name="instagram" type="text" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->instagram" placeholder="Contoh: @username" />
                            </div>

                            <div>
                                <x-input-label for="email" value="Alamat Email (Opsional)" />
                                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->email" />
                            </div>

                            <div>
                                <x-input-label for="motto" value="Motto Hidup" />
                                <x-text-input id="motto" name="motto" type="text" class="mt-1 block w-full text-sm rounded-xl" :value="$pemateri->motto" required />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="alamat" value="Alamat Lengkap" />
                                <textarea id="alamat" name="alamat" rows="3" class="mt-1 block w-full text-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm" required>{{ $pemateri->alamat }}</textarea>
                            </div>

                            <div class="md:col-span-2 flex flex-col sm:flex-row gap-4 items-center border-t border-slate-100 pt-4">
                                <div class="flex-shrink-0">
                                    <p class="text-xs text-gray-500 mb-1.5 font-semibold">Foto Saat Ini:</p>
                                    <img src="{{ $pemateri->foto ? asset('storage/' . $pemateri->foto) : 'https://ui-avatars.com/api/?name='.urlencode($pemateri->nama) }}" class="h-20 w-20 rounded-xl object-cover border border-slate-200 shadow-sm">
                                </div>
                                <div class="flex-1 w-full">
                                    <x-input-label for="foto" value="Ganti Foto Profil (JPG/PNG, Maks 2MB)" />
                    ...
                    @php
                        $eduMap = $pemateri->riwayatPendidikans->keyBy('jenjang');
                    @endphp
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            @foreach(['SD Sederajat', 'SMP Sederajat', 'SMA Sederajat', 'Perguruan Tinggi (S1)', 'Pascasarjana (S2)'] as $tingkat)
                                @php $existingEdu = $eduMap->get($tingkat); @endphp
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                                    ...
                                    <div>
                                        <input type="text" name="pendidikan[{{ $tingkat }}][sekolah]" placeholder="Nama Sekolah / Universitas" 
                                            value="{{ $existingEdu ? $existingEdu->nama_sekolah : '' }}"
                                    ...
                                    <div>
                                        <input type="text" name="pendidikan[{{ $tingkat }}][tahun]" placeholder="Tahun Lulus" 
                                            value="{{ $existingEdu ? $existingEdu->tahun : '' }}"
                                            class="block w-full text-sm rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" />
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="space-y-4" x-data="{ 
                        organisasis: {{ $pemateri->riwayatOrganisasis->isNotEmpty() 
                            ? json_encode($pemateri->riwayatOrganisasis->map(function($org) {
                                return [
                    ...
                    <div class="space-y-4" x-data="{ 
                        pengkaderans: {{ $pemateri->riwayatPengkaderans->isNotEmpty() 
                            ? json_encode($pemateri->riwayatPengkaderans->map(function($pk) {
                                return [
                                    'tingkat' => $pk->tingkat,
                                    'nama' => $pk->nama,
                                    'tahun' => $pk->tahun
